...
import { deleteEmergencyResponsePlanById } from 'apiSdk/emergency-response-plans';
import { deleteIncidentReportById } from 'apiSdk/incident-reports';
...
function FireDepartmentViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<FireDepartmentInterface>(
    ...
    () =>
      getFireDepartmentById(id, {
        relations: ['user', 'emergency_response_plan', 'incident_report', 'resource', 'training_program'],
      }),
  );

  const emergency_response_planHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteEmergencyResponsePlanById(id);
      await mutate();
    } catch (error) {
  ...
  const incident_reportHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteIncidentReportById(id);
      await mutate();
    } catch (error) {
      setDeleteError(error);
    }
              ...
              {hasAccess('emergency_response_plan', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/emergency-response-plans/create?fire_department_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.emergency_response_plan?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/emergency-response-plans/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
                            ...
                            <Td>
                              {hasAccess(
                                ...
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/emergency-response-plans/edit/${record.id}`}
                                >
                              ...
                              {hasAccess(
                                ...
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    emergency_response_planHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
              ...
              {hasAccess('incident_report', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/incident-reports/create?fire_department_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.incident_report?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/incident-reports/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.title}</Td>
                            <Td>{record.description}</Td>
                            <Td>{record.status}</Td>
                            <Td>
                              {hasAccess('incident_report', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/incident-reports/edit/${record.id}`}
                                >
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
                              ...
                              {hasAccess('incident_report', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    incident_reportHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
                            ...
                            <Td>
                              ...
                              {hasAccess('resource', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    resourceHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
                            ...
                            <Td>
                              {hasAccess('training_program', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/training-programs/edit/${record.id}`}
                                >
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
                              ...
                              {hasAccess('training_program', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    training_programHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
