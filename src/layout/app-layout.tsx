...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['fire-chief', 'incident-commander', 'fire-fighter'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<FireDepartmentInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/fire-departments` : null),
    () => getFireDepartments({ tenant_id: session?.user?.tenantId }),
  );
  ...
  const LinkItems: Array<LinkItemProps> = [
...

    {
      ...
      path: isTenantUser ? `/fire-departments/view/${data?.[0]?.id}` : '/fire-departments',
    ...
    { name: 'Resource', path: '/resources', entity: 'resource', service: AccessServiceEnum.PROJECT, icon: FiBox },
    {
